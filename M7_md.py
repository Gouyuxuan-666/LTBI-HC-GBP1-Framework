...
GPU_ID       = "0"  # hardcoded for GTX1080
...
def run_gmx(args, cwd=None, stdin_input=None, timeout=3600):
    ...
    print(f"  [cmd] {' '.join(str(a) for a in args)[:120]}...")
...
def generate_ligand_topology(lig_pdb, lig_sdf):
    ...
    if lig_itp.exists() and lig_gro.exists():
        print(f"[lig] Using existing {lig_itp} and {lig_gro}")
        return lig_itp, lig_gro

    print("[lig] Generating GAFF2 topology via gen_ligand_itp.py...")
    ...
    if gen_script.exists():
        ...
        print(f"[lig] gen_ligand_itp.py not found at {gen_script}")
        print("[lig] Run: python gen_ligand_itp.py ligand.pdb Resveratrol.sdf M7_output/")

    if lig_itp.exists() and lig_gro.exists():
        print(f"[lig] Topology ready: {lig_itp} ({lig_itp.stat().st_size} bytes), "
...
def append_ligand(system_gro, lig_gro, topol):
    ...
    print("[lig] Appending ligand to solvated system...")
    ...
    if 'UNL                  1' not in top_content:
        ...
        print(f"[lig] Topology: +UNL 1 in [molecules]")
    ...
    if n_removed_waters > 0:
        ...
        print("[lig] WARNING: No clashing water found — is ligand outside the box?")

    print(f"[lig] Removed {n_removed_waters} water molecules "
    ...
    print(f"[lig] {n_sys} sys + {n_lig} lig = {n_tot} atoms → {merged_gro}")
